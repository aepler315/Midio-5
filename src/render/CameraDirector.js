...
export class CameraDirector {
  ...
  punch(scale) {
    this.targetZoom = Math.max(this.targetZoom, Math.min(scale, 1.18));
  }
  ...
  update(dtSec, calm) {
    this.zoom += (this.targetZoom - this.zoom) * Math.min(1, dtSec * 14);
    this.targetZoom += (1 - this.targetZoom) * Math.min(1, dtSec * 4);
